-- ===========================
-- AUTO FISH FEATURE V3 - Click Method
-- File: autofishv3.lua
-- ===========================
...
function AutoFishFeature:Init(guiControls)
    ...
    -- Try to get FishingController
    fishingController = self:GetFishingController()
    
    logger:info("Initialized V3 - Click trigger method")
    return true
end

-- Get FishingController module reference
function AutoFishFeature:GetFishingController()
    -- Cache the controller reference
    if not fishingController then
        local success = pcall(function()
            -- Path to FishingController based on decompiled script structure
            local Controllers = ReplicatedStorage:WaitForChild("Controllers", 5)
            local FishingController = require(Controllers:WaitForChild("FishingController", 5))
            fishingController = FishingController
        end)
        
        if not success then
            logger:warn("Failed to get FishingController")
            return nil
        end
    end
...
function AutoFishFeature:Start()
    ...
    spawn(function()
        ...
        -- Step 4: Start click spam until fish caught
...
-- Start spamming Click function from FishingController
function AutoFishFeature:StartCompletionSpam(delay, maxTime)
    ...
    if not textEffectReceived then
        logger:warn("Starting click spam without text effect confirmation")
    end
    
    spamActive = true
    local spamStartTime = tick()
    
    logger:info("Starting click spam...")
    
    spawn(function()
        -- Spam click until fish caught
        while spamActive and isRunning and (tick() - spamStartTime) < maxTime do
            -- Call the game's click function
            self:TriggerFishingClick()
        ...
        if not fishCaughtFlag and (tick() - spamStartTime) >= maxTime then
            logger:warn("⏱️ Click timeout after", maxTime, "seconds")
        end
    end)
end

-- Trigger fishing minigame click using the game's function
function AutoFishFeature:TriggerFishingClick()
    local FishingController = self:GetFishingController()
    if not FishingController then return false end
    
    local success = pcall(function()
        -- Call the game's FishingMinigameClick method
...
function AutoFishFeature:GetStatus()
    return {
        ...
        controllerReady = fishingController ~= nil
    }
end
...
function AutoFishFeature:GetNotificationInfo()
    return {
        ...
        fishCaughtFlag = fishCaughtFlag,
        textEffectReceived = textEffectReceived,
        hasFishingController = fishingController ~= nil
    }
end
